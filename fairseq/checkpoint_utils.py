...
def load_model_ensemble(filenames, arg_overrides=None, task=None, random_net_init=False, re_init_weight=None):
    ...
    """
    ensemble, args, _task = load_model_ensemble_and_task(filenames, arg_overrides, task, random_net_init=random_net_init, re_init_weight=re_init_weight)
    return ensemble, args


def load_model_ensemble_and_task(filenames, arg_overrides=None, task=None, random_net_init=False, re_init_weight=None):
    ...
    for filename in filenames:
        ...
        if not random_net_init:
            model.load_state_dict(state['model'], strict=True)
            print(f'Model Actually load weights from {filename}')
        if re_init_weight is not None:
            renew_state = load_checkpoint_to_cpu(re_init_weight, None)
            model.load_state_dict(renew_state['model'], strict=True)
            print(f'Model Actually load the weight from {re_init_weight}')
