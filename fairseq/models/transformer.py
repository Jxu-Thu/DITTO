...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None, return_attn=False, src_attn_mask=None, incremental_update=True, **unused):
        """
        Args:
        ...
        """
        x, extra = self.extract_features(prev_output_tokens, encoder_out, incremental_state, return_attn, src_attn_mask, incremental_update=incremental_update)
        ...
        for layer in self.layers:
            if incremental_state is None:
                if src_attn_mask is None:
                    ...
                    bsz, T, _ = self_attn_mask.size()
                    self_attn_mask = self_attn_mask.unsqueeze(1)
                    self_attn_mask = self_attn_mask.repeat(1, self.num_heads, 1, 1).view(bsz * self.num_heads, T, T) # B * H * T * T
                ...
                if src_attn_mask is None:
                    ...
                    self_attn_mask = src_attn_mask
                    bsz, T, T_k = self_attn_mask.size()
                    self_attn_mask = self_attn_mask.unsqueeze(1)
                    self_attn_mask = self_attn_mask.repeat(1, self.num_heads, 1, 1).view(bsz * self.num_heads, T,
                                                                                         T_k)  # B * H * T * T
    ...
    def attr_forward(self, prev_output_tokens, tar_layer, tmp_score, encoder_out=None, src_attn_mask=None, incremental_state=None, return_attn=False, **unused):
        """
        Args:
            prev_output_tokens (LongTensor): previous decoder outputs of shape
                `(batch, tgt_len)`, for teacher forcing
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        x, extra = self.extract_features_attr(prev_output_tokens, tar_layer, tmp_score, encoder_out, incremental_state, return_attn, src_attn_mask)
        x = self.output_layer(x)
        return x, extra

    def extract_features_attr(self, prev_output_tokens, tar_layer, tmp_score, encoder_out=None, incremental_state=None, return_attn=False, src_attn_mask=None, **unused):
        """
        Similar to *forward* but only return features.
        ...
        for i_layer, layer in enumerate(self.layers):
            ...
            if incremental_state is None:
                if src_attn_mask is None:
                    ...
                    self_attn_mask = src_attn_mask
                    bsz, T, _ = self_attn_mask.size()
                    bsz_multi = target_batch_size//bsz
                    self_attn_mask = self_attn_mask.unsqueeze(1)
                    self_attn_mask = self_attn_mask.repeat(bsz_multi, self.num_heads, 1, 1).view(target_batch_size * self.num_heads, T, T) # B * H * T * T
                ...
                if src_attn_mask is None:
                    ...
                    bsz_multi = target_batch_size // bsz
                    self_attn_mask = self_attn_mask.unsqueeze(1)
                    self_attn_mask = self_attn_mask.repeat(bsz_multi, self.num_heads, 1, 1).view(target_batch_size * self.num_heads, T,
                                                                                         T_k)  # B * H * T * T
